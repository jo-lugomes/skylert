...
// Configurações do projeto Skylert
...
// Monitoramento de segurança: expulsa ao deslogar
...
// Recupera preferências salvas no Firestore
...
// Mapeamento dos novos elementos do painel de alertas
...
inputCidade.addEventListener('input', async () => {
    ...
    try {
        ...
        const response = await fetch(
            `${API_BASE_URL}/api/cidades?q=${encodeURIComponent(termo)}`
        );
...
document.addEventListener('click', (e) => {
    ...
    if (
        !listaCidades.contains(e.target) &&
        e.target !== inputCidade
    ) {
...
// Fecha a lista de alertas ao clicar fora dela
document.addEventListener('click', (e) => {
    if (
        !listaCidadesAlertas.contains(e.target) &&
        e.target !== alertCidade
    ) {
        listaCidadesAlertas.innerHTML = "";
        listaCidadesAlertas.style.display = "none";
    }
...
// Busca clima padrão de São Paulo ao entrar na tela
window.addEventListener('load', () => {
    buscarClimaReal("Sao Paulo");
});

// Enter na barra de busca também dispara
...
// Chama o Flask e atualiza o HTML
async function buscarClimaReal(cidade, lat = null, lon = null) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Renderiza previsão dos próximos dias
